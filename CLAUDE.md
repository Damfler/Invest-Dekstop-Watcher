...
- **APPDATA путь**: `%APPDATA%\Stack\` (в `.exe`) или рядом со скриптом (в dev)
...
├── app.py             # Главный класс StackTrayApp. pywebview main + pystray detached.
...
├── stack.spec         # PyInstaller spec для сборки .exe.
...
**При сборке .exe:**
- Пользовательские данные: `%APPDATA%\Stack\` (config, cache, logs)
- dashboard.html + icons упакованы внутрь .exe
- Пользователь видит только `Stack.exe`
...
Результат: `dist/Stack.exe` (~50-80 МБ, без Python на машине).

Spec-файл: `stack.spec`. Ключевые настройки:
...
- `APP_NAME` → `"Stack"` (version.py)
- `%APPDATA%\TBankWatcher\` → `%APPDATA%\Stack\` (config.py, wizard.py, updater.py, main.py)
