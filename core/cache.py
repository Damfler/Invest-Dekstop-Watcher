...
def save_cache(portfolios: list, bond_events: list,
               ...
               bond_analytics: list = None):
    """Сохраняет текущее состояние в cache.json."""
    def _dt(v):
        return v.isoformat() if isinstance(v, datetime) else v

    def _ev(e: dict) -> dict:
        return {**e, "date": _dt(e.get("date"))}

    def _ba(b: dict) -> dict:
        r = dict(b)
        if "maturity_date" in r and r["maturity_date"]:
            r["maturity_date"] = _dt(r["maturity_date"])
        return r
    ...
    payload = {
        ...
        "bond_analytics":  [_ba(b) for b in (bond_analytics or [])],
    }
    try:
...
def load_cache() -> dict | None:
    ...
    try:
        ...
        # Восстанавливаем datetime в событиях и аналитике
        from utils.formatting import parse_ts
        for e in data.get("bond_events", []):
            if isinstance(e.get("date"), str):
                e["date"] = parse_ts(e["date"])
        for b in data.get("bond_analytics", []):
            if isinstance(b.get("maturity_date"), str):
                b["maturity_date"] = parse_ts(b["maturity_date"])
